...
export async function getForecastData(query) {
    try {
        ...
        if (query.lat && query.lon) {
            ...
            openWeatherUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=metric&appid=${OPENWEATHER_API_KEY}`;
            weatherApiUrl = `https://api.weatherapi.com/v1/forecast.json?key=${WEATHERAPI_KEY}&q=${city}&days=3`;
        } else {
